...
typedef struct projectile_def_t
{
    ...
    uint8_t gravity           : 3;   // plugin: gravity
    uint8_t invert_gravity    : 1;   // plugin: invert gravity
    ...
    // Replaces the mask this definition tests against a tile when non-zero,
    // so a projectile can react to an extra tile property bit or ignore
    // direction entirely (0 = stock collision, mask tested as normal). Only
    // read when DYNPROJ_ENABLE_OVERRIDE_TILE_COLLISION is on. The CHAIN
    // behaviour never tests tiles at all, so it borrows this byte for its
    // link catch-up speed - the same trick it already plays with `collision`.
    uint8_t override_tile_collision;
    int8_t  amplitude;        // sine / orbit wave
} projectile_def_t;

#define DEF_GRAVITY_MAX  7
#define DEF_GRAVITY(d)   ((d)->invert_gravity ? -((BYTE)((d)->gravity)) : ((BYTE)((d)->gravity)))
